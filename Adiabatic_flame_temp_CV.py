...
def energy_residual(T):
    ...
    n_prod = np.zeros(len(species))
    n_prod[species.index("CO2")] = 12
    n_prod[species.index("H2O")] = 13
    Uprod = internal_energy_mixture(n_prod, T)
...
# ----------------------------
sol = root_scalar(energy_residual, bracket=[1500,7000], method='bisect')
T_ad = sol.root
...
P = (np.sum(n_final) * R * T_ad) / V  # Ideal gas law
...
n_react[species.index("O2")] = 18.5
Uref = internal_energy_mixture(n_react, 300.0)
Uprod_final = internal_energy_mixture(n_final, T_ad)

residual = Uprod_final - Uref
rel_error = residual / abs(Uref)

# ----------------------------------------------------------------
# Display results
# ----------------------------------------------------------------
print("=" * 65)
print("  Constant-Volume Adiabatic Combustion Results ")
print("=" * 65)
print(f"Adiabatic Flame Temperature (T_ad): {T_ad:10.2f} K")
print(f"Equilibrium Pressure (P):          {P/1e5:10.3f} bar\n")

print("Mole Fractions of Major Products:")
print("-" * 50)
for sp, xi in zip(species, X_final):
    if xi > 1e-6:
        print(f"{sp:<6s} : {xi:>12.6e}")
print("-" * 50)

print("\nEnergy Conservation Check:")
print("-" * 50)
print(f"{'U_ref (reactants @300K)':30s} = {Uref:>15.6e} J")
print(f"{'U_prod (products @T_ad)':30s} = {Uprod_final:>15.6e} J")
print(f"{'Residual (U_prod - U_ref)':30s} = {residual:>15.6e} J")
print(f"{'Relative Error':30s} = {rel_error:>15.3e}")
print("-" * 50)
print(" Energy balance verified (should be close to zero).")
print("=" * 65)